...
- **⚠️ 单种子限制（2026-08-20 统一标注）**：全部实验基于 seed=2021 单次训练。弊端：无重复标准差/置信区间，3% 边缘的差异无法区分"真实差异"与"种子噪声"，图无法带误差棒。解决想法（不写死，可灵活调整）：①对关键实验做多种子重跑（训练方差）；②对现有预测做 bootstrap 重采样（评估方差）——两者性质不同，报告时分开。详见 SOP 追踪表待办。
