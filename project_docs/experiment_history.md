...
| Aug EXP-24 (极小值评估) | 三窗口谷点分析 | — | — | — | 谷值区系统性弱；block时间±3月可用；幅度量级级错误 |
...
- 极小值专项评估（阶段3 前置）→ ✅ EXP-24 已完成（2026-08-20，见下）
- 优胜配置 W2/W3 验证

### EXP-24 极小值专项评估（2026-08-20，分析任务）

> 类型：分析任务（复用三窗口已定论 checkpoint，无训练）。预检表跑前写完（含坑回顾行 26/24/25）。
> 数据口径：月均 argmin 为主 + 平滑参照（标注边界效应污染）。对拍全 PASS。

**结果**（5 谷点三口径，详见 EXP-24_experiment.md）：

| 谷点 | 真实谷 | 滚动 | block |
|------|--------|------|-------|
| W1 起点谷 1996-10 | 0.7 | +1月，-6.2⚠️负值 | 无谷，段内最小 4.2 |
| W1 终点谷 2008-08 | 0.3 | 无谷，崩塌-194.2 | -3月，15.3 |
| W2 起点谷 2009-08 | 0.0 | 无谷，55.6 | +1月，31.5 |
| W3 起点谷 2020-02 | 0.2 | +9月，-6.8⚠️负值 | +2月，-2.4⚠️负值 |

**结论**：谷值区=模型系统性最弱区（"接缝弱区"证实）；谷值幅度预测不可靠（量级级错误）；
block 在谷值时间定位上系统性优于滚动（±3月 vs +9/无谷）；阶段3"25 周期最小值"需专门策略。
图：`plots/fig_W1_trough_2008-08_2026-08-20.png`。
