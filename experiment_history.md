...
| **EXP-16** | 08.10 | Stage 7 | **Baseline B参数×1749+数据** | sl132,dm128,nh16,el3,df256,pl12,str6,50ep (ES@12) | **24.02** | **18.07** | **0.553** | ⚠️ 证据不足待对照EXP-16b；滚动33.18（AI结论：≈原BaselineB H1 33.13） | L691 | ✅ 2026-08-10 |
| **EXP-16b** | 08.10 | Stage 7 | **patience 3→20**（EXP-16唯一对照） | sl132,dm128,nh16,el3,df256,pl12,str6,50ep (ES@48) | **24.85** | **13.90** | **0.524** | step0改善18→14 滚动改善33→31 全步微退24→25（过拟合）。峰值仍低估-81.5 | L691 | ✅ 2026-08-10 |
...
## Stage 7: 新 Baseline B 复现 — EXP-16（2026.08.10）

### 目的

在 1749+ 数据上用 Baseline B 参数配置训练，建立与 EXP-14 **同数据源** 的对照基线。解决 Issue #2（Baseline B vs EXP-14 不可比）。

### 配置

| 参数 | EXP-16 | EXP-14 | 原始 Baseline B |
|------|--------|--------|----------------|
...
| patch_len | 12 | 16 | 12 |
| stride | 6 | 8 | 6 |
| epochs | 50 (ES@12) | 10 | 50 |
| d_model | 128 | 128 | 128 |
| enc_in | 3 | 3 | 3 |
| RevIN | 1 | 1 | 1 |

> ⚠️ 非标准消融：与 EXP-14 差 7 项参数。这是 **基线检验** 性质实验，目的不是归因到某个变量，而是检验"Baseline B 配置在 1749+ 数据上是否能工作"。

### 结果

| 指标 | EXP-16 | EXP-14 | dm512 (EXP-13) | 原始 Baseline B |
|------|--------|--------|----------------|----------------|
| step0 MAE | 18.07 | 9.08 | — | 13.02 |
| 全步 MAE | 24.02 | 23.87 | 25.27 | 23.91 |
| R² | 0.553 | 0.568 | 0.539 | — |
| E_r | -85.2 | -75.8 | — | — |
| **滚动 MAE** | **33.18** | 149.92 (崩) | 23.12 | 33.13 |

### 关键发现

1. **滚动 MAE=33.18 ≈ 原始 Baseline B 的 H1=33.13**——即使数据从 1867+ 换成 1749+、ES 提前在 epoch 12 截断，滚动稳定性保持一致。说明"seq=132 + nh16 + el3"的组合是滚动鲁棒性的关键，不是数据范围决定的。

2. step0 MAE=18.07，比 EXP-14（9.08）差近一倍。seq=132 的长序列在安静期输出了更多负值。

3. dm512（seq=96, ep=10）的滚动 MAE=23.12 是最好成绩——大模型 + 短训练 = 输出保守，滚动反而稳。

4. **全步 MAE 和滚动 MAE 不一定正相关**。EXP-14 全步最好（23.87）但滚动最差（149.92）。dm512 全步最差（25.27）但滚动最好（23.12）。

### 配置快照

`configs/EXP-16_2026-08-10.json`（worktree: `EXP-16` 分支）

### 结论

Baseline B 配置在 1749+ 数据上可复现。seq=132 + epochs=50 对滚动至关重要。EXP-14（seq=96, ep=10）不适合做滚动基线。

### 补充：EXP-16b（patience 3→20 单变量对照）

EXP-16 的差主要来自 patience=3 导致的 EaryStopping 提前截断（epoch 12）。patience 调整为 20 后（EXP-16b），训练跑到 epoch 48，step0 从 18.07 改善到 13.90，滚动从 33.18 改善到 30.78。但全步 MAE 从 24.02 轻微退化到 24.85（可能的过拟合），峰值误差仍 -81.5——瓶颈在任务定义/数据范围，非训练轮数可解。

| 指标 | EXP-16 (pat=3) | EXP-16b (pat=20) | 变化 |
|------|---------------|------------------|------|
| step0 MAE | 18.07 | 13.90 | ↓ |
| 全步 MAE | 24.02 | 24.85 | ↑ (微过拟合) |
| R² | 0.553 | 0.524 | ↓ |
| 滚动 MAE | 33.18 | 30.78 | ↓ |
| 峰值 E_r | -85.2 | -81.5 | 微改善 |
| 实际 epochs | 12 | 48 | — |
